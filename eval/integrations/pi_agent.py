...
class PiAgent(BaseInstalledAgent):
    ...
    async def run(
        ...
        info = await self.exec_as_agent(environment, command=shlex.join([
            "/opt/pi-node/bin/node", PREFLIGHT, self.provider, self.model, self.effort,
        ]), env=env)
        ...
        context.metadata = {
            ...
            "project_resources": False, "provider_retries": 0,
            "agent_failed": False,
        }
        failure = None
        try:
            ...
            try:
                ...
                summary = pi_session_summary(
                    session_path.read_text(encoding="utf-8"), provider=self.provider,
