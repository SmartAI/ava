...
export function Composer({
  ...
  const counts = { next_turn: 0, next_step: 0 }
  for (const target of pending.values()) counts[target] += 1
  ...
  >
    ...
    {(counts.next_step > 0 || counts.next_turn > 0) && <div className="flex flex-wrap gap-1.5 px-4.5 text-xs text-faint">
      {counts.next_step > 0 && <span className="rounded-full bg-hover px-2.5 py-1">{counts.next_step} steering for the next step</span>}
      {counts.next_turn > 0 && <span className="rounded-full bg-hover px-2.5 py-1">{counts.next_turn} queued follow-up{counts.next_turn === 1 ? '' : 's'}</span>}
    </div>}
