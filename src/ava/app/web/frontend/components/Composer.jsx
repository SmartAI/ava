...
import { composerCard, iconButton, menu, menuItem, primaryButton, secondaryButton, submitButton, textButton } from '../ui'
...
function PendingRow({ message, disabled, onRevise, onDelete, onSend }) {
  ...
  if (editing) return <div className="flex min-w-0 items-start gap-2 px-2.5 py-2">
    ...
    <textarea
      className="max-h-28 min-h-8 min-w-0 flex-1 resize-y rounded-lg border border-line-strong bg-transparent px-2 py-1 text-[13px] leading-5 text-ink outline-none transition-[border-color,box-shadow] duration-150 focus:border-accent focus:shadow-[0_0_0_3px_color-mix(in_srgb,var(--ava-accent)_20%,transparent)]"
      rows="2"
      value={revision}
    ...
    <div className="flex shrink-0 items-center gap-1">
      <button className={textButton} type="button" disabled={disabled || Boolean(working)} onClick={() => setEditing(false)}>Cancel</button>
      <button className={`${primaryButton} px-2.5 py-1 text-xs`} type="button" disabled={disabled || Boolean(working) || !canSave} onClick={() => run('save', async () => {
        const saved = await onRevise(message.id, revision)
        if (saved) setEditing(false)
        return saved
  ...
  return <div className="flex h-9 min-w-0 items-center gap-2 px-2.5">
    ...
    <div className="flex shrink-0 items-center gap-0.5">
      <button className={textButton} type="button" disabled={disabled || Boolean(working)} onClick={() => setEditing(true)}>Revise</button>
      <button className={`${textButton} text-accent`} type="button" disabled={disabled || Boolean(working)} onClick={() => run('send', () => onSend(message.id))}>{working === 'send' ? 'Sending…' : 'Send now'}</button>
      <button className={`${textButton} hover:text-danger`} type="button" disabled={disabled || Boolean(working)} onClick={() => run('delete', () => onDelete(message.id))}>{working === 'delete' ? 'Deleting…' : 'Delete'}</button>
    </div>
  </div>
}
...
export function Composer({
  ...
  return <div
    className={`${composerCard} flex w-full max-w-[780px] flex-col gap-3 pt-2.5 ${dragging ? 'composer-card--dragging' : ''}`}
  ...
  >
    ...
    {slashOpen && slashCandidates.length > 0 && <div className={`${menu} mx-2.5 p-1`}>{slashCandidates.map((name, index) => <button className={`${menuItem} ${index === slashActive ? 'is-active' : ''}`} aria-selected={index === slashActive} type="button" key={name} onMouseDown={event => { event.preventDefault(); onPickSlash(name) }}><span className="min-w-0 flex-1"><span className="block font-mono text-[13px] text-ink">/{name}</span><span className="block overflow-hidden text-ellipsis whitespace-nowrap text-xs text-faint">{COMMAND_HINTS[name].replace(/^\/\S+(\s+\[\S+\])?\s+/, '')}</span></span></button>)}</div>}
    <textarea
      ...
      aria-label="Message"
    />
    {archived && <div className="flex items-center gap-3 px-2.5 pl-4.5 text-[13px] text-muted"><span className="flex-1">Archived - unarchive to continue</span><button className={`${secondaryButton} px-2.5 py-1 text-xs`} onClick={() => onUnarchive(currentChat)}>Unarchive</button></div>}
    <div className="flex items-center gap-2 px-2.5 pb-2.5">
      <button className={iconButton} title="Attach files" aria-label="Attach files" disabled={unavailable} onClick={() => fileInputRef.current?.click()}><PlusIcon /></button>
      <button className={`${secondaryButton} max-w-80 px-2.5 py-1 text-xs text-quiet hover:text-ink`} title="Invocation directory" onClick={onOpenProjects}><FolderIcon /><span className="min-w-0 overflow-hidden text-ellipsis whitespace-nowrap">{currentProject?.name || 'No project'}</span></button>
      ...
      <button className={`${submitButton} submit-button--${mode}`} title={title} aria-label={title} disabled={archived || submitting || controlling || status === 'aborting'} onClick={() => mode === 'pause' || mode === 'abort' ? onControl(mode) : onEnter(false)}><SubmitIcon mode={mode} /></button>
    </div>
  </div>
}
