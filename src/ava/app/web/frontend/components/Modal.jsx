  ...
  return <div className={`${modalBackdrop} px-4 py-8`} onMouseDown={event => { if (event.target === event.currentTarget) onClose() }}>
    <div className={`${modalCard} max-h-[min(560px,calc(100vh-64px))] w-[min(520px,calc(100vw-32px))]`} ref={dialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="modal-title">
      ...
      <div className={`${modalBody} px-2 pb-2`} ref={listRef}>
        ...
        {!picker && (modal.rows || []).map((item, index) => {
          const Row = item.run ? 'button' : 'div'
          return <Row className={`${item.run ? pickerRow : 'flex w-full px-2 py-[0.4375rem] text-left text-muted'} ${item.selected ? 'bg-selected' : ''}`} key={`${item.label}-${index}`} onClick={item.run}><span className="min-w-0 flex-1"><span className="block overflow-hidden text-ellipsis whitespace-nowrap">{item.label}{item.selected && <span className="text-accent"> ✓</span>}</span><span className="block overflow-hidden text-ellipsis whitespace-nowrap font-mono text-[11px] text-faint">{item.detail || ''}</span></span></Row>
        })}
      </div>
