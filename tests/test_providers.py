...
async def test_deepseek_interleaved_calls_execute_sequentially(
    ...
    def chunk(calls):
        return "data: " + json.dumps({"choices": [{"index": 0, "delta": {"tool_calls": calls}}]}) + "\n\n"

    _Fake.stream = (
        chunk([
            {"index": 0, "id": "write_1", "function": {"name": "write", "arguments": '{"path":"parallel.txt",'}},
...
async def test_openai_rejects_invalid_buffered_calls(fake_server, second_call, finish, error):
    ...
    _Fake.stream = (
        "data: " + json.dumps({"choices": [{"delta": {"tool_calls": calls}}]}) + "\n\n"
    )
    if finish:
